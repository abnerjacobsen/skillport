...
from skillpod.interfaces.mcp.tools import register_tools
from skillpod.modules.indexing import build_index, should_reindex, list_all
...
def _build_instructions(config: Config) -> str:
    base = """# SkillPod: Agent Skills Server

SkillPod provides **Agent Skills** — reusable expert knowledge that loads on demand.
...
def run_server(
    *, config: Config, force_reindex: bool = False, skip_auto_reindex: bool = False
    ...
    mcp = FastMCP("skillpod", version="0.0.0", instructions=instructions)
