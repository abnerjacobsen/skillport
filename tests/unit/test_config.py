...
from skillpod.shared.config import Config, SKILLPOD_HOME
...
class TestConfigDefaults:
    ...
    def test_skills_dir_default(self, monkeypatch):
        """SKILLPOD_SKILLS_DIR defaults to ~/.skillpod/skills."""
        monkeypatch.delenv("SKILLPOD_SKILLS_DIR", raising=False)
        cfg = Config()
        assert cfg.skills_dir == SKILLPOD_HOME / "skills"

    def test_db_path_default(self, monkeypatch):
        """SKILLPOD_DB_PATH defaults to ~/.skillpod/indexes/default/skills.lancedb."""
        monkeypatch.delenv("SKILLPOD_DB_PATH", raising=False)
        cfg = Config()
        assert cfg.db_path == SKILLPOD_HOME / "indexes" / "default" / "skills.lancedb"

    def test_embedding_provider_default(self, monkeypatch):
        """SKILLPOD_EMBEDDING_PROVIDER defaults to 'none'."""
        monkeypatch.delenv("SKILLPOD_EMBEDDING_PROVIDER", raising=False)
        cfg = Config()
        assert cfg.embedding_provider == "none"

    def test_search_limit_default(self, monkeypatch):
        """SKILLPOD_SEARCH_LIMIT defaults to 10."""
        monkeypatch.delenv("SKILLPOD_SEARCH_LIMIT", raising=False)
        cfg = Config()
        assert cfg.search_limit == 10

    def test_search_threshold_default(self, monkeypatch):
        """SKILLPOD_SEARCH_THRESHOLD defaults to 0.2."""
        monkeypatch.delenv("SKILLPOD_SEARCH_THRESHOLD", raising=False)
        cfg = Config()
...
class TestConfigEnvironment:
    ...
    def test_skills_dir_from_env(self, monkeypatch, tmp_path):
        """SKILLPOD_SKILLS_DIR loaded from environment."""
        monkeypatch.setenv("SKILLPOD_SKILLS_DIR", str(tmp_path / "custom-skills"))
        cfg = Config()
        assert cfg.skills_dir == tmp_path / "custom-skills"

    def test_db_path_from_env(self, monkeypatch, tmp_path):
        """SKILLPOD_DB_PATH loaded from environment."""
        monkeypatch.setenv("SKILLPOD_DB_PATH", str(tmp_path / "custom.lancedb"))
        cfg = Config()
        assert cfg.db_path == tmp_path / "custom.lancedb"

    def test_embedding_provider_from_env(self, monkeypatch):
        """SKILLPOD_EMBEDDING_PROVIDER loaded from environment."""
        monkeypatch.setenv("SKILLPOD_EMBEDDING_PROVIDER", "none")
        cfg = Config()
        assert cfg.embedding_provider == "none"

    def test_search_limit_from_env(self, monkeypatch):
        """SKILLPOD_SEARCH_LIMIT loaded from environment."""
        monkeypatch.setenv("SKILLPOD_SEARCH_LIMIT", "50")
        cfg = Config()
        assert cfg.search_limit == 50

    def test_path_expands_tilde(self, monkeypatch):
        """Paths with ~ are expanded."""
        monkeypatch.setenv("SKILLPOD_SKILLS_DIR", "~/my-skills")
        cfg = Config()
...
class TestConfigProviderValidation:
    ...
    def test_openai_requires_key(self, monkeypatch):
        ...
        monkeypatch.setenv("SKILLPOD_EMBEDDING_PROVIDER", "openai")
    ...
    def test_gemini_requires_key(self, monkeypatch):
        ...
        monkeypatch.setenv("SKILLPOD_EMBEDDING_PROVIDER", "gemini")
...
class TestConfigImmutability:
    ...
    def test_with_overrides_creates_new(self, monkeypatch, tmp_path):
        ...
        # Original unchanged
        assert cfg1.skills_dir == SKILLPOD_HOME / "skills"
